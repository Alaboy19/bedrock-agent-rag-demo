...
def chat_with_agent(user_input, agentId, agentAliasId, sessionId):
    client = boto3.client(service_name="bedrock-agent-runtime")
    response = client.invoke_agent(
        agentId=agentId,
        agentAliasId=agentAliasId,  # Required parameter
        sessionId=sessionId,  # Required parameter
    ...
    for event in response["completion"]:
        # print(event)
        if "chunk" in event:
            streamed_response += event["chunk"]["bytes"].decode("utf-8")
            print(streamed_response)

    return streamed_response if streamed_response else "No response from the agent."
                    ...
                    if "observation" in orch_trace:
                        ...
                        if "knowledgeBaseLookupOutput" in obs:
                            ...
                            for ref in obs["knowledgeBaseLookupOutput"].get(
                                ...
                                print(
                                    f"    - {ref['content'].get('text', 'N/A')[:50]}... | File : {ref['metadata']['x-amz-bedrock-kb-source-uri'].split('/')[-1]} | Page : {ref['metadata']['x-amz-bedrock-kb-document-page-number']} "
                                )
